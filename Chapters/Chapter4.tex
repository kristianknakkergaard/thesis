...
\begin{align}
H^\text{int}_{FF} = &\int dxdx' \braket {\hat{\psi}^\dagger_F(x) \hat{\psi}^\dagger_F(x')}  \tilde{V}^\text{ind}_{FF}(x'-x,0) \braket{\hat{\psi}_F(x') \hat{\psi}_F(x)} + \nonumber \\
&\int dxdx' A^\dagger_F(x,x') \tilde{V}^\text{ind}_{FF}(x'-x,0) \braket{\hat{\psi}_F(x') \hat{\psi}_F(x)} + \nonumber \\
&\int dxdx' \braket{\hat{\psi}^\dagger_F(x) \hat{\psi}^\dagger_F(x')} \tilde{V}^\text{ind}_{FF}(x'-x,0) A_F(x',x). \nonumber
\end{align} 
...
\begin{align}
H^\text{int}_{FF,1} &= \int dxdx' \braket {\hat{\psi}^\dagger_F(x) \hat{\psi}^\dagger_F(x')} \tilde{V}^\text{ind}_{FF}(x'-x,0) \braket {\hat{\psi}_F(x') \hat{\psi}_F(x)} \nonumber \\
&= - \frac{1}{\mathcal{L}^2}\sum_{k,k'} \braket {f^\dagger_{k}f^\dagger_{-k} } \braket {f_{k'}f_{-k'}} \int dx dx' \sin(k(x'-x))\sin(k'(x'-x))\tilde{V}^\text{ind}_{FF}(x'-x,0) \nonumber \\
&= - \frac{1}{\mathcal{L}}\sum_{k,k'} \braket {f^\dagger_{k}f^\dagger_{-k}} \braket{f_{k'}f_{-k'}} \int du \sin(ku)\sin(k'u)\tilde{V}^\text{ind}_{FF}(u,0) \nonumber \\
...
As is the case for other mean field theories the ground state has a broken symmetry.\footnote{This e.g. includes ferromagnets and BECs.} By inspecting 
