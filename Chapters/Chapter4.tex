...
In this section we comment on the validity of the performed mean field approximation with offset in the Mermin-Wagner theorem. This is based primarily on the articles \cite{Hohenberg.MerminWagnertheorem, Ortiz.Beyondmeanfieldtheory}. 

The Mermin-Wagner theorem in the context of superfluidity was first proved by P. C. Hohenberg in 1967 \cite{Hohenberg.MerminWagnertheorem}. Hohenberg's original argument is based on $s$-wave pairing in a spinfull fermionic system. However the argument does not depend on this detail. The theorem states, that in one and two dimensions the mean field $\braket{c_kc_{-k}}$ must in fact be vanishing for any nonzero temperature. Therefore, a superfluid phase cannot emerge. However, this very general result only holds for sufficiently short range interactions \cite{Ortiz.Beyondmeanfieldtheory}. Further, in this much more recent article Ortiz and Cobanero investigate an exactly solvable model in one dimension, which has a real space interaction that drops off as $1 / x$, $x$ being the interparticle distance. This is quite similar to the Yukawa interaction in the present context. Ortiz and Cobanero show, that their model supports a gap opening. In turn due to the Landau criterion derived in section \ref{sec.Superfluidity} this means, that a superfluid phase emerges. 

We may therefore speculate, that since the interaction in the present context is also long range, the superfluid phase is not destroyed in one dimension. Further, we may speculate that the analysis based on the mean field approximation will be \textit{qualitatively} correct. Since the model in the present context is not exactly solvable, the only reasonable way to investigate these speculations is to investigate the variance of the order parameter $\braket{c_{-k}c_k}$. Unfortunately, \textit{within} mean field theory this would simply give a zero result. This is of course selfconsistent, but not terribly illuminating in respect to estimating the validity of the approximation. Hence, we cannot use mean field theory to come with an estimate of how well it works. Instead we consider it a possible topic of future work. We will however continue on with this approach encouraged by the result of Ortiz and Cobanero and by the large amount of work done in one dimensional superfluid systems, see e.g. \cite{Alicea, KitaevTopPhases, KitaevQuantumWires, LiYangChen, FuKane2006, GreiterIsingKitaevChain, DeGottardiMajoranaFermions, BudichTopInvMajoranaWires, ZhangWu}. 
