...
\end{equation}
where $n_F = N_F/\mathcal{L}$. For the free fermion gas $k_F = n_{F,0}\pi$. Since we consider $N_F$ and $\mathcal{L}$ as constant $n_F = n_{F,0}$, and so we finally get the second integral equation:
\begin{equation}
...
\end{equation}
Here we have made all of the quantities unitless, denoted by a $\tilde{}$. The energies are expressed in terms of the Fermi energy for the free gas: $\epsilon_{F,0} = \frac{k_F^2}{2m_F}$, and the momentum in terms of the Fermi momentum: $\tilde{k} = k/k_F$. This gives us the required second equation.
...
We are now finally in a position, where we can specify, what was meant by $g_{BF}$ being a small parameter. As seen in table \ref{tab.assumptions}, it means that the corresponding scattering length $a_{BF}$ fulfills the strong inequality $k_F a_{BF} \ll 1$. 
...
\begin{equation}
\tilde{\Delta}_{\tilde{k}} = \frac{2^5}{\pi^2}\frac{n_B}{n_F^3}(k_Fa_B)^2\left(\frac{m_B}{m_F} + \frac{m_F}{m_B} + 2\right) \int \frac{d\tilde{k}'}{2\pi} \ln\left[\frac{(\tilde{k}+\tilde{k}')^2+2/\tilde{\xi}^2}{(\tilde{k}-\tilde{k}')^2+2/\tilde{\xi}^2}\right] \frac{\tilde{\Delta}_{\tilde{k}'}}{2\tilde{E}_{F,\tilde{k}'}}\tanh\left(\frac{\tilde{E}_{F,k}}{2\tilde{T}}\right),
\label{eq.GapequationIntegralUnitless}
\end{equation} 
where $\tilde{\xi} = k_F\xi = \frac{\pi}{\sqrt{8 n_B/n_F^3 k_Fa_B}}$, and $\tilde{E}_{F,\tilde{k}} = \sqrt{(\tilde{k}^2-\tilde{\mu})^2 + |\tilde{\Delta}_{\tilde{k}}|^2}$. The approximation $\mu(T) \approx \epsilon_{F,0}$, then amounts to $\tilde{\mu} \approx 1$. 

\section{Numerical calculation} \label{subsec.pairing.numerical}
...
\begin{figure} 
\begin{center}  
\input{Figures/Deltafreegaschem2/kdepend.tex}  
...
\begin{figure} 
\begin{center}  
\input{Figures/Deltafreegaschem2/Tdepend.tex}  
