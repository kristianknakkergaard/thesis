...
\end{align}
With: $g_1(x) = \frac{1}{N}\text{e}^{-\frac{k_F}{\Delta^{11}}\int_{0}^{x} dx' \mu(x')}$, $g_2(x) = \frac{1}{N}\text{e}^{+\frac{k_F}{\Delta^{11}}\int_{\mathcal{L}}^{x} dx' \mu(x')}$ and $N^2 = \int dx |g_j(x)|^2$ a normalization constant. Further $\Delta^{11} = k_F\left.\frac{\partial \Delta^{11}_k}{\partial k}\right|_{k = 0}$. Hence, $g_1(x)$ and $g_2(x)$ are exponentially located around $x = 0$ and $x = \mathcal{L}$ respectively. The superscript ${}^i$ refers to the wire, the subscript ${}_j$ to which end the wave function is located. 
...
\end{align}
In second quantization the above notion of Kramers partners is described the fact, that: $T_-\gamma^1_{0,j}T_-^{-1} = \gamma^2_{0,j}$, simply because $\psi_{1,F}(x) \overset{T_-}{\to} \psi_{2,F}(x)$ and $T_-$ is antiunitary.
