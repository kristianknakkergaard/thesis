...
\end{bmatrix}. \nonumber
\end{equation}
Now define the norms $A^{\pm}_k = 2 \sqrt{ E^{\pm}_{F,k}(\varepsilon_k + E^{\pm}_{F,k}) }$. With a bit of trial and error, the above requirements combined then result in:
\begin{equation}
U_{F,k} = \begin{bmatrix} 
...
\begin{equation}
\frac{E_0}{N_F \epsilon_{F,0}} = -\frac{1}{8} \int d\tilde{k} \left[\frac{(\tilde{\varepsilon}_{\tilde{k}} - \tilde{E}^{+}_{F, \tilde{k}})^2}{\tilde{E}^{+}_{F, \tilde{k}}} + \frac{(\tilde{\varepsilon}_{\tilde{k}} - \tilde{E}^{-}_{F, \tilde{k}})^2}{\tilde{E}^{-}_{F, \tilde{k}}}\right], \hspace{0.5cm} T = 0. 
...
\end{align}
We further get, that $\Delta^{22}_k = - \Delta^{11}_k$, so that this assumptions is self-consistent. Notice, that the two equations have the exact same structure. This is achieved by defining the effective interwire interaction: 
\begin{equation}
...
\begin{equation}
\sum_k \Delta^{12}_k f^\dagger_{2,k}f^\dagger_{1,-k} \overset{T_j}{\to} \sum_k \Delta^{12*}_k \left(\pm \eta f^\dagger_{2,-k}\right)\left( \eta f^\dagger_{1,k}\right) = \pm \eta^2 \sum_k \Delta^{12*}_{k} f^\dagger_{2,k}f^\dagger_{1,-k}. \nonumber
\end{equation}
The last equality step is achieved by going from $k$ to $-k$ in the sum and using, that $\Delta^{12}_k$ is even in $k$. This shows, that both $\Delta^{12}_k$ real and imaginary realises a time reversal symmetry, which does not exchange the wires, and that these both square to $+\mathbb{I}$. 
