...
\end{equation} 
with $\sigma_1$ the first Pauli matrix and $\eta$ some overall phase. Hence, under this time reversal transformation a fermion from wire 1 is transformed into a fermion in wire 2 and acquires the phase $\eta$. Since $\varepsilon_{1,k} = \varepsilon_{2,k}$, the only problematic terms under time reversal are $\Delta^{11}_k c^\dagger_{1,k}c^\dagger_{1,-k}, \Delta^{22}_k c^\dagger_{2,k}c^\dagger_{2,-k}$ and $\Delta^{12}_kc^\dagger_{2,k}c^\dagger_{1,-k}$. We remember, that time reversal is antiunitary: $TiT^{-1} = -i$. This means, that the first transform according to:
\begin{equation}
...
\begin{equation}
C\begin{bmatrix} c_{j,k} \\ c^\dagger_{j,-k} \end{bmatrix}C^{-1} = \sigma_1 \begin{bmatrix} c^\dagger_{j,-k} \\ c_{j,k} \end{bmatrix} = \begin{bmatrix} c_{j,k} \\ c^\dagger_{j,-k} \end{bmatrix}, 
\end{equation} 
...
%\begin{equation}
%T_1^{\infty} \begin{bmatrix} c_{j,k} \\ c^\dagger_{j,-k} \end{bmatrix} (T_1^{\infty})^{-1} = i\sigma_3\begin{bmatrix} c_{j,-k} \\ c^\dagger_{j,k} \end{bmatrix} = \begin{bmatrix} i c_{j,-k} \\ - i c^\dagger_{j,k} \end{bmatrix}.
%\end{equation} 
...
%\begin{equation}
%T_2^{\infty} \begin{bmatrix} c_{1,k} \\ c_{2,k} \end{bmatrix} (T_2^{\infty})^{-1} = \sigma_2\begin{bmatrix} c_{1,-k} \\ c_{2,-k} \end{bmatrix} = \begin{bmatrix} -i c_{2,-k} \\ i c_{1,-k} \end{bmatrix}.
%\end{equation} 
