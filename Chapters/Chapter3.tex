...
\begin{tikzpicture}
  ...
  \end{feynman}
\end{tikzpicture}
\caption{Feynman diagrams for the induced interaction. Since the interaction is weak, we can neglect all other Feynman diagrams than (1)-(4). Diagrams (1) and (2) stems from the normal Green's function $G_{11}$. Diagrams (3) and (4) stems from the anormalous Green's functions $G_{12}, G_{21}$.} \label{fig.feynmandiagrams}
\end{figure}
...
\begin{equation}
\frac{\tilde{V}_{FF}^\text{ind}(x,0)}{\epsilon_{F,0}} = - 8\left( \frac{m_F}{m_B} + \frac{m_B}{m_F} + 2 \right) \frac{n_B^{1/3}}{n_F}(n_Ba_{BF}^3)^{2/3} \cdot \frac{\text{e}^{ -\frac{ \sqrt{2}\xi }{ |x| } } }{k_F|x|}.
...
\begin{equation}
\frac{2m_F}{k_F}V_{FF}^\text{ind}(q,0) = - 8\left( \frac{m_F}{m_B} + \frac{m_B}{m_F} + 2 \right) \frac{n_B^{1/3}}{n_F}(n_Ba_{BF}^3)^{2/3} \cdot \text{e}^{F(q)} E_1(F(q)).
