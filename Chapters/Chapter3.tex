...
\section{1D-3D induced interaction} \label{sec.1D3Dinducedinteractionsinglewire}
...
\end{equation}
whereby we have expressed the ratio of velocities in terms of unitless quantities and in terms of the coherence length. With this assumption at hand we can focus on the zero frequency induced interaction $V_{\text{ind}}(q,0)$. It is also clear, that the neglect of retardation effects is equivalent to only studying short range interactions: $k_F\xi \lesssim 1$. 
...
\label{eq.Vx_lt=0}
\end{equation}
The result for $V_{\text{ind}}(q,0)$ is shown in figure \ref{fig.Vq} and the result for $\tilde{V}_{\text{ind}}(x,0)$ along with the asymptotic form is shown in figure \ref{fig.Vx}. Both have been done for several values of $l_t$. As expected we see, that the momentum space interaction diverges as $l_t \to 0$, and that the real space interaction asymptotically goes to the Yukawa interaction. The plots are performed by going to a dimensionless form of the interactions; utilizing that the relevant energy scale is the Fermi energy $\epsilon_{F,0} = \frac{k_F^2}{2m_F}$. In detail the asymptotic Yukawa potential on dimensionless form is:
\begin{equation}
...
\end{equation}
Since the momentum space induced interaction is given by the Fourier transform of the induced interaction in real space one unit of $k_F$ is lost to the integration. The dimensionless form is hereby for any $l_t > 0$: 
\begin{equation}
