...
For comparison and later reference, let us list the intra- and interwire induced interactions on unitless form. This we do for $l_t = 0$. In real space we have:
\begin{align}
...
\frac{\tilde{V}_{\text{ind}}^{12}(x,0)}{\epsilon_{F,0}} &= - 8\left( \frac{m_F}{m_B} + \frac{m_B}{m_F} + 2 \right) \frac{n_B^{1/3}}{n_F}(n_Ba_{BF}^3)^{2/3} \cdot \frac{\text{e}^{ - \sqrt{2(x^2 + d^2)} /\xi } }{k_F\sqrt{2(x^2 + d^2)}}. 
\end{align}
