\chapter{Numerical and analytical solutions} % Main chapter title

\label{Chapter8} 

\lhead{Part IV. \emph{Kitaev chain}}
\chead{Chapter 8. \emph{Numerical and analytical solutions}} % This is for the header on each page - perhaps a shortened title
%----------------------------------------------------------------------------------------
...
\section{Analytical solutions} \label{sec.lattice.analyticalsolutions}
In this section we show, that the system exhibit interesting analytical solutions within the mean field approximation. First, we study the infinite coherence length limit. 

\subsection{\texorpdfstring{$\xi \to \infty$}{Infinite range} limit}
The infinite coherence limit can be achieved in the following manner. We let $n_Ba_B \to 0$, so that $\xi \propto 1/\sqrt{n_Ba_B} \to \infty$. However, the strength has the proportionality: $G \propto (n_Ba_{BF})^2/\sqrt{n_Ba_B}$. Therefore, we keep this ratio constant. In this manner the strength of the interaction is held constant, whilst the coherence length goes to infinity. 

The real space interaction is now simply a constant: $\tilde{V}_{\text{ind}}(0,j)/t_{\max} = - G$. The effective interaction hereby simplify considerably:
\begin{align}
\frac{W_{\text{ind}(k,k')}}{t_{\max}} 
&= \frac{1}{2}\sum_j \left[\cos( (k - k')x_j ) - \cos( (k + k')x_j ) \right]\frac{\tilde{V}_{\text{ind}}(0,j)}{t_{\max}} \nonumber \\
&= -\frac{G}{2}\sum_j \left[\cos( (k - k')x_j ) - \cos( (k + k')x_j ) \right] = -\frac{GN}{2}\left[\delta_{k,k'} - \delta_{k, -k'}\right]. \nonumber
\end{align}
Inserting this into the gap equation yields:
\begin{equation}
\Delta_k = \frac{G}{2E_k}\Delta_k \Rightarrow \Delta_k = 0 \text{ or } |\Delta_k|^2 = \left(\frac{G}{2}\right)^2 - \varepsilon_k^2
\end{equation}
Hence, we have an analytical solution for the pairing. In the regions where $|\varepsilon_k| < G / 2$ the last expression is positive, and this solution is self-consistent. Otherwise, $\Delta_k = 0$. 
