...
double WFF0(double k, double kprime, double BBscatlength)
{
...
main (void)
{
  ...
  /*BBscatlengths:*/
  double BBscatlength_low = 0.0001, BBscatlength_up = 0.3, dBBscatlength = 0.0002;
  ...
  /*For calculating mu:*/
  double mu_low = 0.935, dmu = 0.00001;
  ...
  for (double BBscatlength = BBscatlength_up; BBscatlength > BBscatlength_low; BBscatlength-=dBBscatlength)
  {
    mu_check = 0;
    for (int i = 0; i < N; ++i)
    {
      ...
      for (int iprime = 0; iprime < N; ++iprime)
      {
        ...
        gsl_matrix_set( WFF0matrix, i, iprime, WFF0(k, kprime, BBscatlength) );
      }
    }
    for (int j = 0; j < 50; ++j)
    {
    ...
    printf("%lg \t %lg \t %lg \t %lg \n", BBscatlength, gsl_vector_max(D), mu_min_value, 0.0);
  }
