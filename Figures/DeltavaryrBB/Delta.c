...
{
  /*variables */
  double rBB_low = 0.001, rBB_up = 0.01, drBB = 0.0001; /*(nB * aBB^3)^(1/3) <= 0.03 atmost! (1 percent depletion) */
  double rBF = 0.036;    /*(nB * aBF^3)^(1/3) */
  double mB  = 7.0/40.0; /*mB/mF*/
  
  double inverseBdist = 0.0286;
  ...
  printf("%s \t %s \t %s \t %s \t %s \n", "rBB", "Delta", "mu", "aB", "retneg");
  printf("\n \n");

  /*rBB dependency for T = 0*/
  for (rBB = rBB_up; rBB > rBB_low; rBB-=drBB)
  {
    ...
    fprintf(stderr, "%i \t %i\n", ++iter_number, check);
    printf("%.4lg \t %.4lg \t %.4lg \t %.4lg \t %.4lg \n", rBB, gsl_vector_max(D), mu_min_value, aB, retneg);
  }
  
  fprintf(stderr, "(nBaBF^3)^(1/3) = %lg, mB/mF = %lg, nB/nF^3 = %lg, kF*aB = %lg, kF*aBF = %lg \n", rBF, mB, nB, aB, aBF);
