...
main (void)
{
  ...
  /*k-values:*/
  double k_low = 0.0, k_up = 100.0, dk = 0.01;
  ...
  double muintegral, vkprimenorm2, ukprimenorm2;
  ...
  printf("%s \t %s \t %s \t %s \t %s \t %s \n", "k", "Occupancy", "mu", "kmax", "Energy", "Mean field validity" );
  ...
  for (int r = 0; r < Nr; ++r)
  {
    ...
    for (int iprime = 0; iprime < N_plot; ++iprime)
    {
      ...
      vkprimenorm2  = 1.0 / 2.0 * ( 1.0 - epsilonkprime / EFkprime );
      ukprimenorm2  = 1.0 / 2.0 * ( 1.0 + epsilonkprime / EFkprime ); 

      meanfieldval  =  vkprimenorm2 / ukprimenorm2;
      printf("%lg \t %lg \t %lg \t %lg \t %lg \t %lg \n", kprime, vkprimenorm2, mu, kmax, EFkprime, meanfieldval);
    }
    printf("\n \n");
    fprintf(stderr, "%i \t %i\n", ++check, iter);
  }
