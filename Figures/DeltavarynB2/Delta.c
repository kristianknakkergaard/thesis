...
double WFF0(double k, double kprime, double rBB, double rBF, double inverseBdist, double mB)
{
...
{
  /*variables */
  double inverseBdist_low = 0.004, inverseBdist_up = 0.2, dinverseBdist = 0.0001; /*(nB * aBB^3)^(1/3) <= 0.03 atmost! (1 percent depletion) */
  ...
  /*k-values:*/
  double k_low = 0.0, k_up = 70.0, dk = 0.05;
  ...
  for (double inverseBdist = inverseBdist_up; inverseBdist > inverseBdist_low; inverseBdist-=dinverseBdist)
  {
    nB     = pow(inverseBdist, -3.0);
    ...
    for (int i = 0; i < N; ++i)
    {
      ...
      for (int iprime = 0; iprime < N; ++iprime)
      {
        ...
        gsl_matrix_set( WFF0matrix, i, iprime, WFF0(k,kprime, rBB, rBF, inverseBdist, mB) );
      }
    }
    ...
    fprintf(stderr, "%i \t %i\n", ++iter_number, check);
    if (retneg < 20.0 || gsl_vector_max(D) < 1e-2)
    {
      break;
    }
    printf("%.4lg \t %.4lg \t %.4lg \t %.4lg \t %.4lg \t %.4lg \n", inverseBdist, gsl_vector_max(D), mu_min_value, aB, retneg, aBF);
  }
