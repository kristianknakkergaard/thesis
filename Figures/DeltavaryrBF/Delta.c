...
{
  /*variables */
  double rBF_low = 0.01, rBF_up = 0.04, drBF = 0.00005; /*(nB * aBB^3)^(1/3) <= 0.03 atmost! (1 percent depletion) */
  double rBB = 0.01;    /*(nB * aBF^3)^(1/3) */
  ...
  printf("%s \t %s \t %s \t %s \t %s \n", "rBF", "Delta", "mu", "aB", "retneg");
  printf("\n \n");

  /*rBB dependency for T = 0*/
  for (rBF = rBF_up; rBF > rBF_low; rBF-=drBF)
  {
    ...
    fprintf(stderr, "%i \t %i\n", ++iter_number, check);
    printf("%.4lg \t %.4lg \t %.4lg \t %.4lg \t %.4lg \n", rBF, gsl_vector_max(D), mu_min_value, aB, retneg);
  ...
  fprintf(stderr, "(nBaB^3)^(1/3) = %lg, mB/mF = %lg, nB/nF^3 = %lg, kF*aB = %lg, kF*aBF = %lg \n", rBB, mB, nB, aB, aBF);
