...
main (void)
{
  ...
  /*free parameters:*/
  double xi_low   = 1.0,  dxi   = 0.5; /*we will eventually want to control the range rather than the Bose gas parameter*/
  double fill_low = 0.04, dfill = 0.05;
  double t2       = 10.0;
  int    N        = 100; 
  ...
  gsl_matrix *fillximatrix  = gsl_matrix_calloc(Nfill, Nxi);

  /*for the chemical potential*/
  double mu_low     = -2.0, dmu = 0.005;
  double mu         = 0.5;
  int Nmu           = 800;
  ...
  for (int jxi = 0; jxi < Nxi; ++jxi)
  {
    ...
    for (int jfill = 0; jfill < Nfill; ++jfill)
    {
      ...
      for (int j = 0; j < 1000; ++j)
      {
        ...
        if ( fabs(mu - mu_min_value)/mu_min_value < 1e-4 && fabs(gsl_vector_max(D) - D_maxk)/D_maxk < 1e-4 && pow(fill - fillingnum, 2.0) )
        {
          break;
        }

        if ( fabs(mu) < 1e-4 && fabs(gsl_vector_max(D) - D_maxk)/D_maxk < 1e-4 && pow(fill - fillingnum, 2.0) )
        {
          break;
        }
      ...
      gsl_matrix_set(fillximatrix, jfill, jxi, CS1);
      fprintf(stderr, "checkxi = %i, \t checkfill = %i \n", checkxi, ++checkfill);
    }
    ++checkxi;
  }
  ...
  for (int jxi = 0; jxi < Nxi; ++jxi)
  {
    for (int jfill = 0; jfill < Nfill; ++jfill)
    {
      printf("%lg \t ", fabs( gsl_matrix_get( fillximatrix, jfill, jxi) ) );
    }
    printf("\n");
  }
