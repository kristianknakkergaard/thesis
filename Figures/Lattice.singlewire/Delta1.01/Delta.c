...
main (void)
{
  ...
  /*free parameters:*/
  double xi_low   = 1.0, xi_up    = 11.0,  dxi  = 0.1; /*we will eventually want to control the range rather than the Bose gas parameter*/
  double fill_low = 0.0, fill_up  = 1.0, dfill  = 0.01, fill_middle = 0.5;
  double t2       = 1.0;
  int    N        = 100; 
  ...
  gsl_vector *Dguess      = gsl_vector_calloc(N);
  ...
  for (double xi = xi_low; xi <= xi_up; xi += dxi)
  {
    /*derived parameters:*/
    ...
    /*fill loop, fill < 1/2:*/
    for (double fill = fill_middle; fill > fill_low - dfill; fill -= dfill)
    {
    ...
    /*fill loop, fill > 1/2:*/
    for (double fill = fill_middle + dfill; fill < fill_up + dfill; fill += dfill)
    {
      mu = -3.0 + 2.0 * 3.0 * fill, check = 0;
