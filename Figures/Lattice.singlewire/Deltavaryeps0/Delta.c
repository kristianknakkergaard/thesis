...
main (void)
{
  ...
  fprintf(stderr, "(nBaB^3)^(1/3) = %lg, (nBaBF^3)^(1/3) = %lg, mB/mF = %lg, nB*a^3 = %lg, xi / a = %lg, vF/c0 = %lg \n", rBB, rBF, mB, nB, xi, vFoverc0);
  fprintf(stderr, "\n \n");
  ...
  printf("%s \t %s \t %s \t %s \t %s \t %s \n", "eps0", "CS1", "Deltamax", "Nmu/N", "Emin", "check" );


  /*T = 0:*/
  for (double eps0 = eps0_low; eps0 < eps0_up; eps0+=deps0)
  {
    for (int i = 0; i < N; ++i)
    {
      k = ((double)i) * dk + k_low;
    ...
    printf("%lg \t %lg \t %lg \t %lg \t %lg \t %i \n", eps0, CS1, gsl_vector_max(D), muintegral, gsl_vector_min(E), check );

    fprintf(stderr, "eps0counter = %i, total = %lg \n", ++eps0counter, (eps0_up - eps0_low) / deps0);
  }
