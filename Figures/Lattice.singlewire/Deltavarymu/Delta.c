...
main (void)
{
  ...
  double mu_low = -1.0, mu_up = 2.0, dmu = 0.005;
  ...
  printf("%s \t %s \t %s \t %s \t %s \t %s \n", "mu", "CS1", "Deltamax", "Nmu/N", "Emin", "check" );


  /*T = 0:*/
  for (double mu = mu_low; mu < mu_up; mu+=dmu)
  {
    for (int i = 0; i < N; ++i)
    {
      k = ((double)i) * dk + k_low;
    ...
    printf("%lg \t %lg \t %lg \t %lg \t %lg \t %i \n", mu, CS1, gsl_vector_max(D), muintegral, gsl_vector_min(E), check );

    fprintf(stderr, "mucounter = %i, total = %lg \n", ++mucounter, (mu_up - mu_low) / dmu);
  }
