...
main (void)
{
  ...
  double d_low = 0.74, d_up = 0.775, dd = 0.00005, d_middle = 0.759; 
  ...
  int d_iter = 0;
  /*T = 0: */
  printf("%s \t %s \t %s \t %s \t %s \t %s \n", "d", "Delta11kF", "Delta12kF", "E0", "mu", "check");
  printf("\n \n");

  for (double d = d_middle; d > d_low ; d-= dd)
  {
    for (int i = 0; i < N; ++i)
    {
      k = ((double)i) * dk + k_low;
    ...
    for (int iprime = 0; iprime < N; ++iprime)
    {
      ...
      if (iprime < N - 1)
      {
        dDelta11      = (gsl_vector_get(D11, iprime + 1) - gsl_vector_get(D11, iprime) ) / dk;
        CS11integrand = 1.0/(2.0 * M_PI) * (Delta11kprime * (2.0 * kprime) - epsilonkprime * dDelta11)/(EFkprime * EFkprime);
        CS11         += dk * CS11integrand; 
      }
    }

    printf("%lg \t %lg \t %lg \t %lg \t %lg \t %lg \t %i \n", d, gsl_vector_get(D11, kF_index), gsl_vector_get(D12, kF_index), E0integral, mu, CS11, check);
    fprintf(stderr, "d_iter = %i, \t D11kF = %lg, \t D12kF = %lg \n", ++d_iter, gsl_vector_get(D11, kF_index), gsl_vector_get(D12, kF_index));
  }
  printf("\n \n");
  ...
  for (double d = d_middle; d < d_up ; d+= dd)
  {
    for (int i = 0; i < N; ++i)
    {
      k = ((double)i) * dk + k_low;
