...
{
  return 0.2 * sin(k) / (pow(k, 4.0) + 1);
}

double Deltaguess12(double k)
{
  return 0.2 * cos(k) / (pow(k, 4.0) + 1); 
}
...
main (void)
{
  ...
  double d_low = 0.745, d_up = 0.748, dd = 0.0001;
  ...
  for (double d = d_low; d < d_up; d+=dd)
  {
    ...
    for (int i = 0; i < N; ++i)
    {
      ...
      gsl_vector_set(D12real, i, 1.08 * Deltaguess12(k));
    ...
    {
      k = ((double)i) * dk + k_low;
      fprintf(stderr, "%lg \t %lg \t %lg \n", k, gsl_vector_get(D11, i), gsl_vector_get(D12real, i)  );
    }
