...
main (void)
{
  ...
  double d_low = 0.74, d_up = 0.775, dd = 0.0025; 
  ...
  double x_low = -50.0, x_up = 50.0, dx = 0.05; 
  ...
  double pairwavefunction11;
  double pairwavefunction12;
  double pairwavefunctionintegrand11;
  double pairwavefunctionintegrand12;
  ...
  for (double d = d_low; d < d_up; d+=dd)
  {
    ...
    {
      pairwavefunction11 = 0.0;
      pairwavefunction12 = 0.0; 
      for (int iprime = 0; iprime < N; ++iprime)
      {
        kprime        = ((double)iprime) * dk + k_low;
        epsilonkprime = kprime * kprime - mu;
        Delta11kprime = gsl_vector_get(D11, iprime);
        Delta12kprime = gsl_vector_get(D12, iprime);
        EFkprime      = sqrt(epsilonkprime * epsilonkprime + Delta11kprime * Delta11kprime + Delta12kprime * Delta12kprime );

        pairwavefunctionintegrand11 = 1.0 / (2.0 * M_PI) * sin(kprime * x) * Delta11kprime / (2.0 * EFkprime);
        pairwavefunctionintegrand12 = 1.0 / (2.0 * M_PI) * cos(kprime * x) * Delta12kprime / (2.0 * EFkprime);

        pairwavefunction11 += dk * pairwavefunctionintegrand11;
        pairwavefunction12 += dk * pairwavefunctionintegrand12;

      }
      printf("%lg \t %lg \t %lg \t %lg \t %i \n", x, pairwavefunction11, pairwavefunction12, mu, check);
    }
