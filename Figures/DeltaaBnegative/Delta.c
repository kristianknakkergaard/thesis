...
{
  double BBscatlength = -0.1; /*kF * aB*/
  double densityratio = 1.0;   /*nB/nF^3 */
  double xi2          = M_PI * M_PI/( 8.0 * densityratio * BBscatlength ); /*xi * kF*/ 
  ...
  double f = - factor * log( ( pow(k+kprime, 2.0) + 2.0/(xi2) )/( pow(k-kprime, 2.0) + 2.0/(xi2) ) );
  return f;
}
...
{
  /*k-values:*/
  double k_low = 0.0, k_up = 100.0, dk = 0.05;
  ...
  for (double T = TC; T < T_high; T+=dT)
  {
    ...
    do
    {
      ...
      if (status == GSL_SUCCESS)
        fprintf(stderr, "%lg \t %lg \t %lg \t %lg \t %lg \t %i \n", T, 0.0, 0.0, mufree_guess, mufree_guess, check);
    }
