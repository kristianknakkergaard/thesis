...
double WFF12plus(double k, double kprime, double d, double rBB, double rBF, double nB, double mB)
{
  return 1.0 / 2.0 * ( VFF12(k - kprime, d, rBB, rBF, nB, mB) + VFF12(k + kprime, d, rBB, rBF, nB, mB)  ); 
}

double WFF12minus(double k, double kprime, double d, double rBB, double rBF, double nB, double mB)
{
  return 1.0 / 2.0 * ( VFF12(k - kprime, d, rBB, rBF, nB, mB) - VFF12(k + kprime, d, rBB, rBF, nB, mB)  );
}
...
main (void)
{
  ...
  double d_low = 0.05, d_up = 0.15, dd = 0.02; 
  ...
  gsl_matrix *WFF12plusmatrix  = gsl_matrix_calloc(N, N);
  gsl_matrix *WFF12minusmatrix = gsl_matrix_calloc(N, N);
  ...
  for (double d = d_low; d < d_up; d+=dd)
  {
    ...
    for (int i = 0; i < N; ++i)
    {
      ...
      gsl_vector_set(D12, i, Deltaguess11(k));

      for (int iprime = 0; iprime < N; ++iprime)
      {
        kprime = ((double)iprime) * dk + k_low;
        gsl_matrix_set( WFF12plusmatrix, i, iprime, WFF12plus(k, kprime, d, rBB, rBF, nB, mB) );
        gsl_matrix_set( WFF12minusmatrix, i, iprime, WFF12minus(k, kprime, d, rBB, rBF, nB, mB) );
      }
    }
    for (int j = 0; j < 1000; ++j)
    {
      ...
      for (int i = 0; i < N; ++i)
      {
        ...
        for (int iprime = 0; iprime < N; ++iprime)
        {
          ...
          EFkprime        = pow(epsilonkprime * epsilonkprime + pow( Delta11kprime + Delta12kprime, 2.0), 1.0 / 2.0);
          
          gapintegrand11  =  -1.0 / (2.0 * M_PI) * gsl_matrix_get(WFF11matrix, i, iprime) * Delta11kprime / (2.0 * EFkprime);
          gapintegrand12  =  -1.0 / (2.0 * M_PI) * gsl_matrix_get(WFF12minusmatrix, i, iprime) * Delta12kprime / (2.0 * EFkprime);
          gapintegrandcheck = gsl_matrix_get(WFF12plusmatrix, i, iprime) * Delta11kprime / (2.0 * EFkprime); 
      ...
      if ( fabs(mu - mu_min_value)/mu_min_value < convergence && fabs(gsl_vector_max(Dcheck)) < convergence && fabs(gsl_vector_max(D11) - D11_maxk)/D11_maxk < convergence && fabs(gsl_vector_max(D12) - D12_maxk)/D12_maxk < convergence)
      {
        break;
      }

      if ( fabs(mu - mu_min_value)/mu_min_value < convergence && fabs(gsl_vector_max(Dcheck)) < convergence && fabs(gsl_vector_max(D11) - D11_maxk)/D11_maxk < convergence && gsl_vector_max(D12) < convergence)
      {
        break;
      }

      if ( fabs(mu - mu_min_value)/mu_min_value < convergence && fabs(gsl_vector_max(Dcheck)) < convergence && gsl_vector_max(D11) < convergence && fabs(gsl_vector_max(D12) - D12_maxk)/D12_maxk < convergence)
      {
        break;
      }

      if ( fabs(mu - mu_min_value)/mu_min_value < convergence && fabs(gsl_vector_max(Dcheck)) < convergence && gsl_vector_max(D11) < convergence && gsl_vector_max(D12) < convergence)
      {
        break;
      }
      mu = mu_min_value;
    ...
    {
      k = ((double)i) * dk + k_low;
      printf("%lg \t %lg \t %lg \t %lg \t %lg \t %i \n", k, gsl_vector_get(D11,i), gsl_vector_get(D12,i), mu, E0integral + 2.0 * mu, check);
    }
    printf("\n \n");
  }
  ...
  gsl_matrix_free(WFF12plusmatrix);
  gsl_matrix_free(WFF12minusmatrix);
