...
main (void)
{
  ...
  fprintf(stderr, "\n \n");
  fprintf(stderr, "%s \t %s \t %s \t %s \t %s\n", "corfunc", "aveordparam", "ratio", "T", "check" );
  fprintf(stderr, "\n \n");
  ...
  printf("%s \t %s \t %s \t %s \t %s\n", "k", "corfuncintegrand", "aveordparamintegrand", "T", "check" );
  printf("\n \n");
  ...
  for (int iprime = 0; iprime < N; ++iprime)
  {
    ...
    corfuncintegrand     = pow( 1.0 / 2.0 * ( 1.0 - epsilonkprime / EFkprime ), 2.0 );
    aveordparamintegrand = pow( Deltakprime / (2.0 * EFkprime), 2.0);

    if (kprime >= sqrt(mu - 0.001) && kprime <= sqrt(mu + 0.001) )
    {
      corfunc     += corfuncintegrand * dk;
      aveordparam += aveordparamintegrand * dk;
    }

    printf("%lg \t %lg \t %lg \t %lg \t %i\n", kprime, corfuncintegrand, aveordparamintegrand, 0.0, check);
  }
  printf("\n \n");

  fprintf(stderr, "%lg \t %lg \t %lg \t %lg \t %i\n", corfunc, aveordparam, corfunc/aveordparam, 0.0, check);
  ...
  for (int iT = 0; iT < numberT; ++iT)
  {
    ...
    for (int iprime = 0; iprime < N; ++iprime)
    {
      ...
      corfuncintegrand     = pow( epsilonkprime / EFkprime * 1.0 / (exp(EFkprime / T[iT]) + 1.0) + 1.0 / 2.0 * ( 1.0 - epsilonkprime / EFkprime ), 2.0 );
      aveordparamintegrand = pow( Deltakprime / (2.0 * EFkprime) * tanh(EFkprime / (2.0 * T[iT])) , 2.0);

      if (kprime >= sqrt(mu - T[iT]) && kprime <= sqrt(mu + T[iT]) )
      {
        corfunc     += corfuncintegrand * dk;
        aveordparam += aveordparamintegrand * dk;
      }

      printf("%lg \t %lg \t %lg \t %lg \t %i\n", kprime, corfuncintegrand, aveordparamintegrand, T[iT], check);
    }
    printf("\n \n");

    fprintf(stderr, "%lg \t %lg \t %lg \t %lg \t %i\n", corfunc, aveordparam, corfunc / aveordparam, T[iT], check);
  }
