...
main (void)
{
  ...
  for (double x = - x_up; x < x_up; x+=dx)
  {
    ...
    for (int iprime = 0; iprime < N; ++iprime)
    {
      ...
      Deltakprime      = gsl_vector_get(D, iprime);
      EFkprime         = sqrt(epsilonkprime * epsilonkprime + Deltakprime * Deltakprime);
      corfuncintegrand = 1.0 / M_PI * sin(kprime * x) * Deltakprime / (2.0 * EFkprime);
  ...
  for (int iT = 0; iT < numberT; ++iT)
  {
    ...
    for (double x = - x_up; x < x_up; x+=dx)
    {
      ...
      for (int iprime = 0; iprime < N; ++iprime)
      {
        ...
        Deltakprime      = gsl_vector_get(D, iprime);
        EFkprime         = sqrt(epsilonkprime * epsilonkprime + Deltakprime * Deltakprime);
        corfuncintegrand = 1.0 / M_PI * sin(kprime * x) * Deltakprime / (2.0 * EFkprime) * tanh(EFkprime / (2.0 * T[iT]));
        corfunc         += corfuncintegrand * dk;
      }
      fprintf(stderr, "%lg \t %lg \t %lg \t %i \n", x, corfunc, T[iT], check);
    }
